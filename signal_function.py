...
class SignalFunc():
    ...
    def signal_prepare(self):
        ...
        # 开始处理
        temp_sig_matrix_0 = frame_data[0:data_len:4]
        temp_sig_matrix_1 = frame_data[1:data_len:4]
        temp_sig_matrix_2 = frame_data[2:data_len:4]
        temp_sig_matrix_3 = frame_data[3:data_len:4]
        # 存入文件
        # ch_1_path = r'F:\python_workspace\dataTrans\dataRecord\ch_1.txt'
        # ch_2_path = r'F:\python_workspace\dataTrans\dataRecord\ch_2.txt'
        # ch_3_path = r'F:\python_workspace\dataTrans\dataRecord\ch_3.txt'
        # ch_4_path = r'F:\python_workspace\dataTrans\dataRecord\ch_4.txt'
        # with open(ch_1_path, 'a') as file_obj:
        #     for val in temp_sig_matrix_0:
        #         file_obj.write(str(val)[1:-1] + '\n')
        # with open(ch_2_path, 'a') as file_obj:
        #     for val in temp_sig_matrix_1:
        #         file_obj.write(str(val)[1:-1] + '\n')
        # with open(ch_3_path, 'a') as file_obj:
        #     for val in temp_sig_matrix_2:
        #         file_obj.write(str(val)[1:-1] + '\n')
        # with open(ch_4_path, 'a') as file_obj:
        #     for val in temp_sig_matrix_3:
        #         file_obj.write(str(val)[1:-1] + '\n')
        # 重构
        samples_in_chirp = self.__samples_in_chirp
        chirps_in_frame = self.__chirps_in_frame
        temp_sig_matrix_0 = np.reshape(temp_sig_matrix_0, (chirps_in_frame, samples_in_chirp))
        temp_sig_matrix_1 = np.reshape(temp_sig_matrix_1, (chirps_in_frame, samples_in_chirp))
        temp_sig_matrix_2 = np.reshape(temp_sig_matrix_2, (chirps_in_frame, samples_in_chirp))
        temp_sig_matrix_3 = np.reshape(temp_sig_matrix_3, (chirps_in_frame, samples_in_chirp))
        self.__data_obj.update_sig_2D_matrix_channel_0(temp_sig_matrix_0)
        self.__data_obj.update_sig_2D_matrix_channel_1(temp_sig_matrix_1)
        self.__data_obj.update_sig_2D_matrix_channel_2(temp_sig_matrix_2)
        self.__data_obj.update_sig_2D_matrix_channel_3(temp_sig_matrix_3)
    ...
    def cfar_detect(self):
        ...
        target_amp = []
        # 开始计算
        expand_matrix = self.create_expand_matrix(predetection_RD_matrix)
        ...
        for i in range(M, M+half_M):
            ...
            if range_detect_flag:
                # j=range(half_N, N+half_N)是全部多普勒轴
                for j in range(half_N, N+half_N):
